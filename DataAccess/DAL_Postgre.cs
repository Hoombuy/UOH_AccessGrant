...
namespace DataAccess
{
    ...
    /// 该类用于与Oracle数据库通讯
    /// </summary>
    public class DAL_Postgre : DAL_Base
    {
        private NpgsqlDataAdapter Da;
        private NpgsqlCommand SelectCommand;

        //构造函数
        public DAL_Postgre(DataConnectionInfo P_DC)
        {
            DC = P_DC;
            this.Da = new NpgsqlDataAdapter();
            this.SelectCommand = new NpgsqlCommand();
            this.Da.SelectCommand = this.SelectCommand;
            this.SelectCommand.Connection = (NpgsqlConnection)this.DC.myConn;
        }
        ...
        public DataTable GetDataTable(string ProduceName, ArrayList TheParamesterList)
        {
            ...
            {
                this.SelectCommand.Parameters.Add(new NpgsqlParameter(((DataContract.DataParamester)TheParamesterList[i]).PName, ((DataContract.DataParamester)TheParamesterList[i]).PValue));
            }
        ...
        public override bool Validate(string Oracle)
        {
            ...
            {
                this.DC.myConn.Open();
                NpgsqlDataReader Read = this.SelectCommand.ExecuteReader();
                if (Read.Read())
                {
                    Value = true;
                }
        ...
        {
            NpgsqlDataAdapter OracleDa = new NpgsqlDataAdapter();
            ...
            OracleDa.InsertCommand = (NpgsqlCommand)InsertCom;
            OracleDa.UpdateCommand = (NpgsqlCommand)UpdateCom;
            OracleDa.DeleteCommand = (NpgsqlCommand)DeleteCom;
            try
            {
        ...
        public override bool Excute(ArrayList Oracle, ArrayList PName, ArrayList PValue, bool IsTransaction)  //使用事务的批量数据提交
        {
            ...
            {
                this.DC.myConn.Open();
                NpgsqlTransaction myTrans = ((NpgsqlConnection)this.DC.myConn).BeginTransaction();
                try
                {
                    for (int j = 0; j < Oracle.Count; j++)
                    {
                        ...
                        TCommand.Connection = (NpgsqlConnection)this.DC.myConn;
